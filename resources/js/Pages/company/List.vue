    ...
    <AppLayout title="Ambiente">
            ...
            <div class="card-body">
                <div class="d-md-flex gap-4 align-items-center">
                    ...
                    <div class="dropdown ms-auto">
                        <Link :href="route('environment.create')" class="btn btn-primary btn-icon">
        ...
        <div class="table-responsive">
            <table id="users" class="table table-custom table-lg">
                ...
                <tr v-for="ambiente in ambientes.data" :key="ambiente.id">
                    <td>
                        <input class="form-check-input" type="checkbox">
                    </td>
                    <td>{{ ambiente.id }}</td>
                    <td><Link :href="route('environment.edit', ambiente.id)" >{{ ambiente.nome }}</Link></td>
                    <td>{{ formatarData(ambiente.data_cadastro) }}</td>
                    <td>{{ formatarData(ambiente.data_update) }}</td>
                    <td>
                        <span v-if="ambiente.status == 1" class="badge bg-success">Ativo</span>
                    ...
                    <td class="text-end">
                        <div class="dropdown">
                            ...
                            <div class="dropdown-menu dropdown-menu-end">
                                <Link :href="route('environment.edit', ambiente.id)" class="dropdown-item"><i class="bi bi-pencil"></i> Editar</Link>
                                <button @click="destroy(ambiente.id)" class="dropdown-item text-danger"><i class="bi bi-trash"></i> Remover</button>
                            </div>
                        </div>
                    </td>
                </tr>
        ...
        <nav class="mt-4" aria-label="Page navigation example">
            <ul class="pagination justify-content-end">
                <!-- Botão "Previous" -->
                <li class="page-item" :class="{ disabled: !ambientes.prev_page_url }">
                ...
                <template v-if="ambientes.last_page > 1">
                    <!-- Calcular o conjunto limitado de páginas exibidas -->
                    <template v-for="page in limitedPages" :key="page">
                        <li class="page-item" :class="{ active: page === ambientes.current_page }">
                ...
                <!-- Botão "Next" -->
                <li class="page-item" :class="{ disabled: !ambientes.next_page_url }">
...
function updateTable(page = 1){
    ...
    router.visit(`/environment/list?${query}`,{preserveState: true, replace: true, preserveScroll: true})
}

// Calcula as páginas a serem exibidas, limitando a um conjunto específico
const limitedPages = computed(() => {
    const currentPage = props.ambientes.current_page;
    const lastPage = props.ambientes.last_page;
...
function destroy(id){
    try{
        if(confirm('Tem certeza que deseja remover o registro?')){
            Inertia.delete(route('environment.delete', id));
