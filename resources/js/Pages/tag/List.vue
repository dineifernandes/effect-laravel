    ...
    <AppLayout title="Etiqueta">
            ...
            <div class="card-body">
                <div class="d-md-flex gap-4 align-items-center">
                    ...
                    <div class="dropdown ms-auto">
                        <Link :href="route('tag.create')" class="btn btn-primary btn-icon">
        ...
        <div class="table-responsive">
            <table id="users" class="table table-custom table-lg">
                ...
                <tr>
                    ...
                    <th @click="sortBy('duracao_etiqueta')">Duração
                        <span v-if="props.sort == 'duracao_etiqueta'">
                        <span v-if="props.sortDir == 'asc'"><i class="bi bi-arrow-down"></i></span>
                        <span v-if="props.sortDir == 'desc'"><i class="bi bi-arrow-up"></i></span>
                    </span>
                    </th>
                ...
                <tr v-for="etiqueta in etiquetas.data" :key="etiqueta.id">
                    ...
                    <td>{{ etiqueta.id }}</td>
                    <td><Link :href="route('tag.edit', etiqueta.id)" >{{ etiqueta.nome }}</Link></td>
                    <td>{{ etiqueta.duracao_etiqueta }} dia(s)</td>
                    <td>{{ formatarData(etiqueta.data_cadastro) }}</td>
                    <td>{{ formatarData(etiqueta.data_update) }}</td>
                    <td>
                        <span v-if="etiqueta.status == 1" class="badge bg-success">Ativo</span>
                    ...
                    <td class="text-end">
                        <div class="dropdown">
                            ...
                            <div class="dropdown-menu dropdown-menu-end">
                                <Link :href="route('tag.edit', etiqueta.id)" class="dropdown-item"><i class="bi bi-pencil"></i> Editar</Link>
                                <button @click="destroy(etiqueta.id)" class="dropdown-item text-danger"><i class="bi bi-trash"></i> Remover</button>
                            </div>
                        </div>
                    </td>
                </tr>
        ...
        <nav class="mt-4" aria-label="Page navigation example">
            <ul class="pagination justify-content-end">
                <!-- Botão "Previous" -->
                <li class="page-item" :class="{ disabled: !etiquetas.prev_page_url }">
                ...
                <template v-if="etiquetas.last_page > 1">
                    <!-- Calcular o conjunto limitado de páginas exibidas -->
                    <template v-for="page in limitedPages" :key="page">
                        <li class="page-item" :class="{ active: page === etiquetas.current_page }">
                ...
                <!-- Botão "Next" -->
                <li class="page-item" :class="{ disabled: !etiquetas.next_page_url }">
...
function updateTable(page = 1){
    ...
    router.visit(`/tag/list?${query}`,{preserveState: true, replace: true, preserveScroll: true})
}

// Calcula as páginas a serem exibidas, limitando a um conjunto específico
const limitedPages = computed(() => {
    const currentPage = props.etiquetas.current_page;
    const lastPage = props.etiquetas.last_page;
...
function destroy(id){
    try{
        if(confirm('Tem certeza que deseja remover o registro?')){
            Inertia.delete(route('tag.delete', id));
