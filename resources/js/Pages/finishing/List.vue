    ...
    <AppLayout title="Acabamentos">
            ...
            <div class="card-body">
                <div class="d-md-flex gap-4 align-items-center">
                    ...
                    <div class="dropdown ms-auto">
                        <Link :href="route('finishing.create')" class="btn btn-primary btn-icon">
        ...
        <div class="table-responsive">
            <table id="users" class="table table-custom table-lg">
                ...
                <tr v-for="acabamento in acabamentos.data" :key="acabamento.id">
                    <td>
                        <input class="form-check-input" type="checkbox">
                    </td>
                    <td>{{ acabamento.id }}</td>
                    <td><Link :href="route('finishing.edit', acabamento.id)" >{{ acabamento.nome }}</Link></td>
                    <td>{{ formatarData(acabamento.data_cadastro) }}</td>
                    <td>{{ formatarData(acabamento.data_update) }}</td>
                    <td>
                        <span v-if="acabamento.status == 1" class="badge bg-success">Ativo</span>
                    ...
                    <td class="text-end">
                        <div class="dropdown">
                            ...
                            <div class="dropdown-menu dropdown-menu-end">
                                <Link :href="route('finishing.edit', acabamento.id)" class="dropdown-item"><i class="bi bi-pencil"></i> Editar</Link>
                                <button @click="destroy(acabamento.id)" class="dropdown-item text-danger"><i class="bi bi-trash"></i> Remover</button>
                            </div>
                        </div>
                    </td>
                </tr>
        ...
        <nav class="mt-4" aria-label="Page navigation example">
            <ul class="pagination justify-content-end">
                <!-- Botão "Previous" -->
                <li class="page-item" :class="{ disabled: !acabamentos.prev_page_url }">
                ...
                <template v-if="acabamentos.last_page > 1">
                    <!-- Calcular o conjunto limitado de páginas exibidas -->
                    <template v-for="page in limitedPages" :key="page">
                        <li class="page-item" :class="{ active: page === acabamentos.current_page }">
                ...
                <!-- Botão "Next" -->
                <li class="page-item" :class="{ disabled: !acabamentos.next_page_url }">
...
function updateTable(page = 1){
    ...
    router.visit(`/finishing/list?${query}`,{preserveState: true, replace: true, preserveScroll: true})
}

// Calcula as páginas a serem exibidas, limitando a um conjunto específico
const limitedPages = computed(() => {
    const currentPage = props.acabamentos.current_page;
    const lastPage = props.acabamentos.last_page;
...
function destroy(id){
    try{
        if(confirm('Tem certeza que deseja remover o registro?')){
            Inertia.delete(route('finishing.delete', id));
