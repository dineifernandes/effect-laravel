<template>
    ...
    <AppLayout title="Matérias Prima">
        <div class="card">
            <div class="card-body">
                <div class="d-md-flex gap-4 align-items-center">
                    ...
                    <div class="dropdown ms-auto">
                        <Link :href="route('feedstock.create')" class="btn btn-primary btn-icon">
        ...
        <div class="table-responsive">
            <table id="users" class="table table-custom table-lg">
                ...
                <tr v-for="materia in materias.data" :key="materia.id">
                    ...
                    <td>{{ materia.id }}</td>
                    <td><Link :href="route('feedstock.edit', materia.id)" >{{ materia.nome }}</Link></td>
                    <td>{{ formatarData(materia.data_cadastro) }}</td>
                    <td>{{ formatarData(materia.data_update) }}</td>
                    <td>
                        <span v-if="materia.status == 1" class="badge bg-success">Ativo</span>
                    ...
                    <td class="text-end">
                        <div class="dropdown">
                            ...
                            <div class="dropdown-menu dropdown-menu-end">
                                <Link :href="route('feedstock.edit', materia.id)" class="dropdown-item"><i class="bi bi-pencil"></i> Editar</Link>
                                <button @click="destroy(materia.id)" class="dropdown-item text-danger"><i class="bi bi-trash"></i> Remover</button>
                            </div>
                        </div>
                    </td>
                </tr>
        ...
        <nav class="mt-4" aria-label="Page navigation example">
            <ul class="pagination justify-content-end">
                <!-- Botão "Previous" -->
                <li class="page-item" :class="{ disabled: !materias.prev_page_url }">
                ...
                <template v-if="materias.last_page > 1">
                    <!-- Calcular o conjunto limitado de páginas exibidas -->
                    <template v-for="page in limitedPages" :key="page">
                        <li class="page-item" :class="{ active: page === materias.current_page }">
                ...
                <!-- Botão "Next" -->
                <li class="page-item" :class="{ disabled: !materias.next_page_url }">
...
function updateTable(page = 1){
    ...
    router.visit(`/feedstock/list?${query}`,{preserveState: true, replace: true, preserveScroll: true})
}

// Calcula as páginas a serem exibidas, limitando a um conjunto específico
const limitedPages = computed(() => {
    const currentPage = props.materias.current_page;
    const lastPage = props.materias.last_page;
...
function destroy(id){
    try{
        if(confirm('Tem certeza que deseja remover o registro?')){
            Inertia.delete(route('feedstock.delete', id));
