<template>
    ...
    <AppLayout title="Acessório/Material">
        <div class="card">
            <div class="card-body">
                <div class="d-md-flex gap-4 align-items-center">
                    ...
                    <div class="dropdown ms-auto">
                        <Link :href="route('acessorymaterial.create')" class="btn btn-primary btn-icon">
        ...
        <div class="table-responsive">
            <table id="users" class="table table-custom table-lg">
                ...
                <tr v-for="acessorio in acessorios.data" :key="acessorio.id">
                    <td>
                        <input class="form-check-input" type="checkbox">
                    </td>
                    <td>{{ acessorio.id }}</td>
                    <td><Link :href="route('acessorymaterial.edit', acessorio.id)" >{{ acessorio.nome }}</Link></td>
                    <td>{{ formatarData(acessorio.data_cadastro) }}</td>
                    <td>{{ formatarData(acessorio.data_update) }}</td>
                    <td>
                        <span v-if="acessorio.status == 1" class="badge bg-success">Ativo</span>
                    ...
                    <td class="text-end">
                        <div class="dropdown">
                            ...
                            <div class="dropdown-menu dropdown-menu-end">
                                <Link :href="route('acessorymaterial.edit', acessorio.id)" class="dropdown-item"><i class="bi bi-pencil"></i> Editar</Link>
                                <button @click="destroy(acessorio.id)" class="dropdown-item text-danger"><i class="bi bi-trash"></i> Remover</button>
                            </div>
                        </div>
                    </td>
                </tr>
        ...
        <nav class="mt-4" aria-label="Page navigation example">
            <ul class="pagination justify-content-end">
                <!-- Botão "Previous" -->
                <li class="page-item" :class="{ disabled: !acessorios.prev_page_url }">
                ...
                <template v-if="acessorios.last_page > 1">
                    <!-- Calcular o conjunto limitado de páginas exibidas -->
                    <template v-for="page in limitedPages" :key="page">
                        <li class="page-item" :class="{ active: page === acessorios.current_page }">
                ...
                <!-- Botão "Next" -->
                <li class="page-item" :class="{ disabled: !acessorios.next_page_url }">
...
function updateTable(page = 1){
    ...
    router.visit(`/acessorymaterial/list?${query}`,{preserveState: true, replace: true, preserveScroll: true})
}

// Calcula as páginas a serem exibidas, limitando a um conjunto específico
const limitedPages = computed(() => {
    const currentPage = props.acessorios.current_page;
    const lastPage = props.acessorios.last_page;
...
function destroy(id){
    try{
        if(confirm('Tem certeza que deseja remover o registro?')){
            Inertia.delete(route('acessorymaterial.delete', id));
